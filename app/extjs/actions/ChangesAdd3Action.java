...
public final class ChangesAdd3Action extends Action {
    ...
    public ActionForward execute(ActionMapping mapping,
        ...
        if (linOther.equalsIgnoreCase("on")) {
        } else {
            linOther = noNull(request.getParameter("lin11"));
        }
        String linOtherTask = noNull(request.getParameter("linOtherText"));
        ...
        if (dtpOther.equalsIgnoreCase("on")) {
        } else {
            dtpOther = noNull(request.getParameter("dtp7"));
        }
        String dtpOtherTask = noNull(request.getParameter("dtpOtherText"));
        ...
        if (engOther.equalsIgnoreCase("on")) {
        } else {
            engOther = noNull(request.getParameter("eng8"));
        }
        String engOtherTask = noNull(request.getParameter("engOtherText"));
        ...
        int maxChangeNo = ProjectHelper.selectMaxChangeNoForProject(Integer.valueOf(projectViewId).intValue());
        String changeNo = request.getParameter("changeNo");
         if (StandardCode.getInstance().noNull(changeNo).equalsIgnoreCase("-1")) {
          
        }else if (!StandardCode.getInstance().noNull(changeNo).equalsIgnoreCase("")) {
            maxChangeNo = Integer.parseInt(changeNo);
        }
        ...
        for (Iterator iterSources = currentProject.getSourceDocs().iterator(); iterSources.hasNext();) {
            ...
            ////System.out.println("maxChangeNo="+maxChangeNo+", sd.changeNo="+sd.getChangeNo().intValue());
       if(sd.getChangeNo().intValue()==maxChangeNo){     
            for (Iterator iterTargets = sd.getTargetDocs().iterator(); iterTargets.hasNext();) {

                TargetDoc td = (TargetDoc) iterTargets.next();
                if (td.getChangeNo().intValue() == maxChangeNo) {
                    ...
                    for (int i = 0; i < inspections.length; i++) {
                        ...
                        i2.setInspector("");
                        i2.setNote("");
                        i2.setLanguage((String) LanguageAbs.getInstance().getAbs().get(td.getLanguage()));
                    ...
                    if (lin1.equals("on")) { //if checked in form, then add this task to target Doc
                        ...
                        if (!StandardCode.getInstance().noNull(changeNo).equalsIgnoreCase("-1")) {
                            lt.setChangeDesc("C" + maxChangeNo);
                        }
                        linTasks.add(lt);
...
//                                   System.err.println("");
                    ...
                    if (dtp1.equals("on")) { //if checked in form, then add this task to target Doc
                        ...
                        if (!StandardCode.getInstance().noNull(changeNo).equalsIgnoreCase("-1")) {
                            dt.setChangeDesc("C" + maxChangeNo);
                        }
                        dtpTasks.add(dt);
                    }
                    ...
                    if (eng1.equals("on")) { //if checked in form, then add this task to target Doc
                        ...
                        if (!StandardCode.getInstance().noNull(changeNo).equalsIgnoreCase("-1")) {
                            et.setChangeDesc("C" + maxChangeNo);
                        }
                        engTasks.add(et);
                    }
                    ...
                    if (linOther.equals("on")) { //if checked in form, then add this task to target Doc
                        //System.out.println("inside lin1");
                        OthTask lt = new OthTask();
                        lt.setSourceLanguage(sd.getLanguage());
                        lt.setTargetLanguage(td.getLanguage());
                        lt.setTargetDoc(td);
                        lt.setTaskName(linOtherTask);
                        lt.setOrderNum(new Integer(1));
                        lt.setPostQuote("true");
                        //Auto set rate fee
                        if (clp != null) {
                            for (int z = 0; z < clp.length; z++) {
                                if (clp[z].getSource() != null && clp[z].getSource().equals(lt.getSourceLanguage()) && clp[z].getTarget() != null && clp[z].getTarget().equals(lt.getTargetLanguage())
                                        && clp[z].getTask() != null && clp[z].getTask().equals("OTH - " + lt.getTaskName())) {
                    ...
                    if (dtpOther.equals("on")) { //if checked in form, then add this task to target Doc
                        //System.out.println("inside lin1");
                        OthTask lt = new OthTask();
                        lt.setSourceLanguage(sd.getLanguage());
                        lt.setTargetLanguage(td.getLanguage());
                        lt.setTargetDoc(td);
                        lt.setTaskName(dtpOtherTask);
                        lt.setOrderNum(new Integer(2));
                        lt.setPostQuote("true");
                        //Auto set rate fee
                        if (clp != null) {
                    ...
                    if (engOther.equals("on")) { //if checked in form, then add this task to target Doc
                        //System.out.println("inside lin1");
                        OthTask lt = new OthTask();
                        lt.setSourceLanguage(sd.getLanguage());
                        lt.setTargetLanguage(td.getLanguage());
                        lt.setTargetDoc(td);
                        lt.setTaskName(engOtherTask);
                        lt.setOrderNum(new Integer(3));
                        lt.setPostQuote("true");
                        //Auto set rate fee
                        if (clp != null) {
