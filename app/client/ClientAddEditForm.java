//ClientAddEditForm.java works to collect/display values
//to/from the web pages (jsp) related to a new client
...
public class ClientAddEditForm extends ActionForm
{
		...
		HttpServletRequest request)
{
		 ...
		 *
		 */
                if (Company_name == null || Company_name.length() < 1 || Company_name.equals(""))
		{

                        
                      //  errors.add("Comapny_name",new ActionError("Comapny_name"));
			errors.add("Comapny_name", new ActionMessage("errors.required", "Company Name"));

		}

                //do not allow duplicate client names <code>Company_name</code>
                if(ClientService.getInstance().getSingleClientByName(Company_name) != null) {
                    errors.add("Comapny_name", new ActionMessage("errors.duplicateCompanyName"));
                }
