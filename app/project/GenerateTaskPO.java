 ...
 * @author Niteshwar
 */
public class GenerateTaskPO extends Action{
...
//(new java.io.File("C:/"+projectId)).mkdir();
        ...
        String Po6Tier="6";
        if(!p.getCompany().isScaleDefault()){Po6Tier="";}
        //get invoice
...
String formFileName="";
String tasktype="";
                            LinTask lt=ProjectService.getInstance().getSingleLinTask(Integer.parseInt(linId));
 lt.setPoNumber(poNumber);
                           ProjectService.getInstance().updateLinTask(lt);
                           flagT=0;flagP=0;flagE=0;
                           if(lt.getTaskName().contentEquals("Translation")){flagT=1;}
                           if(lt.getTaskName().contentEquals("Editing")){flagE=1;}
                           if(lt.getTaskName().trim().contentEquals("Proofreading")||lt.getTaskName().contentEquals("Proofreading / Linguistic QA")){flagP=1;}
if(StandardCode.getInstance().noNull(lt.getMulti()).equalsIgnoreCase("")){




 if(!p.getCompany().isScaleDefault()){
                if(flagT==1&&flagE==0&&flagP==0){formFileName="PO_Translator.pdf";tasktype="Translation";}
                if(flagT==0&&flagE==1&&flagP==0){formFileName="PO_Editor.pdf";tasktype="Editing";}
                if(flagT==0&&flagE==0&&flagP==1){formFileName="PO_Proofreader.pdf";tasktype="Proofreading";}
                if(flagT==0&&flagE==0&&flagP==0) {formFileName="PO_Editor.pdf";tasktype=lt.getTaskName();}
 }else{
                if(flagT==1&&flagE==0&&flagP==0){formFileName="PO_Translator6.pdf";tasktype="Translation";}
                if(flagT==0&&flagE==1&&flagP==0){formFileName="PO_Editor6.pdf";tasktype="Editing";}
                if(flagT==0&&flagE==0&&flagP==1){formFileName="PO_Proofreader6.pdf";tasktype="Proofreading";}
                if(flagT==0&&flagE==0&&flagP==0) {formFileName="PO_Editor6.pdf";tasktype=lt.getTaskName();}
 }
}else{

 if(!p.getCompany().isScaleDefault()){
                if(lt.getMulti().equalsIgnoreCase("T&E")){formFileName="PO_TE.pdf";tasktype="T&E";}
     else if(lt.getMulti().equalsIgnoreCase("T&P"))
     {
         formFileName = "PO_TP.pdf";
         tasktype = "T&P";
     }
     else if(lt.getMulti().equalsIgnoreCase("E&P"))
     {
         formFileName = "PO_EP.pdf";
         tasktype = "E&P";
     }
 }else{
                if(lt.getMulti().equalsIgnoreCase("T&E")){formFileName="PO_TE6.pdf";tasktype="T&E";}
     else if(lt.getMulti().equalsIgnoreCase("T&P"))
     {
     ...
     else if(lt.getMulti().equalsIgnoreCase("E&P"))
     {
         ...
         tasktype = "E&P";
                ...
                //PdfStamper stamp = new PdfStamper(reader, new FileOutputStream("C:/"+projectId+"/"+p.getNumber() + p.getCompany().getCompany_code() + "-" + pdfNamePoNo +"-"+pdfNameLng+"-"+tasktype+ ".pdf"));
              ...
              try{
                ...
                }}}catch(Exception e){form1.setField("contractor"," " );}
            ...
            double scaleNew = 1.0;
            ...
            }else{totalAmount = costTotalNew + word100Cost + wordRepCost + wordRep8599+wordContext+wordPerfect;}
                ...
                form1.setField("Task2", "Proofreading");
                 ...
                 stamp.close();
            pdfNameLng =  pdfNameLng.replaceAll(" ", "_").replaceAll("-", "_");
          String strHeader ="attachment;Filename="+p.getNumber() + p.getCompany().getCompany_code() + "-" + pdfNameLng+"-"+tasktype+ ".pdf";
...
//                 File inpt=new File("C:/"+projectId+"/"+p.getNumber() + p.getCompany().getCompany_code() + "-" + pdfNamePoNo +"-"+pdfNameLng+"-"+tasktype+ ".pdf");
