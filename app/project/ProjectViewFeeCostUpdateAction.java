...
//        subTotal=subTotal-pmPercentDollarTotalDoubleFromDB;
        ...
        if(StandardCode.getInstance().noNull(change).isEmpty()){
        ...
        Change1 c = ProjectService.getInstance().getSingleChange(id, change);
            c.setPmPercentDollarTotal(pmPercentDollarTotal);
            c.setPmPercent(pmPercent);
            c.setRushPercentDollarTotal(rushPercentDollarTotal);
            c.setRushPercent(rushPercent);
            c.setDiscountDollarTotal(discountDollarTotal);
...
//            Change1 change1 = ProjectService.getInstance().getSingleChange(id, entry.getKey());
//            if(change1 == null){
...
//                pmRate = 0;
//            }
//            pmPercentDollarTotalDouble += +(pmRate / 100) * entry.getValue();
