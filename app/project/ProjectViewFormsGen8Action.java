//ProjectViewFormsGen8Action.java creates the
//Instructions for linguist pdf
...
public final class ProjectViewFormsGen8Action extends Action {
	...
	projectId = request.getParameter("projectViewId");
       ...
       Resource r=null;
        ...
        User u = UserService.getInstance().getSingleUserRealName(StandardCode.getInstance().getFirstName(p.getPm()), StandardCode.getInstance().getLastName(p.getPm()));
            ...
            //START process pdf
            try {
                PdfReader reader = new PdfReader("C:/templates/LI05_001.pdf"); //the template
                ...
                form1.setField("pmname", p.getPm());                
                form1.setField("pmemail", u.getWorkEmail1());
                
                if(u.getWorkPhoneEx() != null && u.getWorkPhoneEx().length() > 0) { //ext present
                    form1.setField("pmphone", StandardCode.getInstance().noNull(u.getWorkPhone()) + " ext " + StandardCode.getInstance().noNull(u.getWorkPhoneEx()));
                }
                else { //no ext present
                    form1.setField("pmphone", StandardCode.getInstance().noNull(u.getWorkPhone()));
                }
...
//                    over.addImage(img, 45, 0,0, 45, 100, 300);
//                }
//                //END add images
                ...
                if((r.getFirstName().length() >= 1 && r.getFirstName() != null) && (r.getLastName().length() >= 1 && r.getLastName() != null)) {
                    response.setHeader("Content-disposition", "attachment; filename=" + StandardCode.getInstance().noNull(r.getFirstName()) + "_" + StandardCode.getInstance().noNull(r.getLastName()) + "-LinguistInstructions" + ".pdf");
                }
                else {
                    response.setHeader("Content-disposition", "attachment; filename=" + StandardCode.getInstance().noNull(r.getCompanyName()) + "-LinguistInstructions" + ".pdf");
                }}else{
                  response.setHeader("Content-disposition", "attachment; filename=LinguistInstructions" + ".pdf");
