//ProjectViewFormsGen9Action.java creates the
//RFQ for linguists pdf
...
public final class ProjectViewFormsGen9Action extends Action {
...
//        Resource r = ResourceService.getInstance().getSingleResource(Integer.valueOf(lt.getPersonName()));
            ...
            //START process pdf
            try {
                PdfReader reader = new PdfReader("C:/templates/LI07_001.pdf"); //the template
...
//                }
                ...
                if(u.getPicture() != null && u.getPicture().length() > 0) {
                    ...
                    over.addImage(img, 60, 0,0, 70, 46,556);
                }
                //END add images
                ...
                response.setHeader("Content-disposition", "attachment; filename=" + "RFQ-for-Linguist" + ".pdf");
