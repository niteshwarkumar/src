...
import app.resource.RateScoreDtp;
 ...
 * @author niteshwar
 */
public class ProjectViewTeamPrefVendorUpdateLin extends Action {
    ...
    public ActionForward execute(ActionMapping mapping,
        ...
        for (int count = 0; count < counter; count++) {
            ...
            String VendorCheckT = request.getParameter("VendorCheckT" + String.valueOf(count));
            String VendorCheckE = request.getParameter("VendorCheckE" + String.valueOf(count));
            String VendorCheckP = request.getParameter("VendorCheckP" + String.valueOf(count));
            String VendorCheckTE = request.getParameter("VendorCheckTE" + String.valueOf(count));
            String VendorCheckMF = request.getParameter("VendorCheckMF" + String.valueOf(count));
            
        
            
                if (VendorCheckT != null || VendorCheckE != null || VendorCheckP != null || VendorCheckTE != null 
                        || VendorCheckMF != null  ) {

                //get resource and task to bind
                String resourceId = request.getParameter("ResourceId" + String.valueOf(count));
                String linIdT = request.getParameter("Translation" + String.valueOf(count));
...
//                 t.add(ProjectService.getInstance().getSingleLinTask(Integer.valueOf(linIdT)));
                if(!StandardCode.getInstance().noNull(linIdE).equalsIgnoreCase(""))
                 taskList.add(ProjectService.getInstance().getSingleLinTask(Integer.valueOf(linIdE)));
                 if(!StandardCode.getInstance().noNull(linIdP).equalsIgnoreCase(""))
                 taskList.add(ProjectService.getInstance().getSingleLinTask(Integer.valueOf(linIdP)));
                 if(!StandardCode.getInstance().noNull(linIdT).equalsIgnoreCase(""))
                 taskList.add(ProjectService.getInstance().getSingleLinTask(Integer.valueOf(linIdT)));
                //bind the task to the resource (team member)
                for(int tl = 0; tl<taskList.size(); tl++) {
                    LinTask t = (LinTask)taskList.get(tl);
