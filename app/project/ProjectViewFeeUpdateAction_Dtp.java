...
public final class ProjectViewFeeUpdateAction_Dtp extends Action {
...
//            }//END process FEE values
        ...
        for(int i = 0; i < dtpTasks.length; i++) {
            ...
            if(dt.getTotal() != null) { //if from delete then make sure it is not null
                ...
                try{
                    ...
                    }else{dtpCurrencyTotal+=thisTotal;}
  ...
  //          ProjectService.getInstance().updateOthTask(ot);
        ...
        double subTotal = linTotal + engTotal + dtpCurrencyTotal + othTotal;
...
//        }
        ...
        String sourceOs=(String) qvg.get("sourceOs");
        String sourceApplication=(String) qvg.get("sourceApplication");
        String sourceVersion=(String) qvg.get("sourceVersion");
        String deliverableOs=(String) qvg.get("deliverableOs");
        String deliverableApplication=(String) qvg.get("deliverableApplication");
        String deliverableVersion=(String) qvg.get("deliverableVersion");
        String deliverableSame=request.getParameter("deliverableSame");
        String sourceTechNotes=(String) qvg.get("sourceTechNotes");
        String deliverableTechNotes=(String) qvg.get("deliverableTechNotes");
       // String versionNotes=(String) qvg.get("versionNotes");
        //String sourceOs=(String) qvg.get("discountDollarTotal");
        try{
        if(deliverableSame.equalsIgnoreCase("on")){
        deliverableOs=sourceOs;
        deliverableApplication=sourceApplication;
        deliverableVersion=sourceVersion;
            }
        } catch (Exception e) {
        }
        p.setSourceOS(StandardCode.getInstance().noNull(sourceOs));
        p.setSourceApplication(StandardCode.getInstance().noNull(sourceApplication));
        p.setSourceVersion(StandardCode.getInstance().noNull(sourceVersion));
        p.setDeliverableApplication(StandardCode.getInstance().noNull(deliverableApplication));
        p.setDeliverableOS(StandardCode.getInstance().noNull(deliverableOs));
        p.setDeliverableVersion(StandardCode.getInstance().noNull(deliverableVersion));
        p.setSourceTechNotes(sourceTechNotes);
        p.setDeliverableTechNotes(deliverableTechNotes);
      //  p.set
       // p.setSourceTechNotes(sourceVersion)
