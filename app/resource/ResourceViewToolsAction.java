//ResourceViewToolsAction.java gets the current team object (resource)
//from the db.  It then places that resource into the resource form for
//display in the jsp page featuring tools. It then places location values into
//cookies
...
public final class ResourceViewToolsAction extends Action {
	...
	resourceId = request.getParameter("resourceViewId");
        ...
        Set resourceTools = r.getResourceTools();
        
        //prepare the resourceTools for display
        ResourceTool[] resourceToolsArray = (ResourceTool[]) resourceTools.toArray(new ResourceTool[0]);
        
        rvt.set("resourceTools", resourceToolsArray);
        rvt.set("resourceFromTools", r);
        
        MALinguistic linMA = ResourceService.getInstance().getSingleLinMA(id); 
        rvt.set("linMA", linMA);
        
        MADtpEng dtpMA = ResourceService.getInstance().getSingleDtpEngMA(id); 
        rvt.set("dtpMA", dtpMA);
        
        MAExpert expertMA = ResourceService.getInstance().getSingleExpertMA(id); 
        rvt.set("expertMA", expertMA);
        
        MAOther othMA = ResourceService.getInstance().getSingleOtherMA(id); 
        rvt.set("otherMA", othMA);
        ...
        response.addCookie(StandardCode.getInstance().setCookie("resourceViewTab", "Tools"));
