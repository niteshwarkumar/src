//ResourceViewSpecialtiesAddSpecific2Action.java gets the list
//of specialties to add for this resource
//many-to-many link
...
public final class ResourceViewSpecialtiesAddSpecific2Action extends Action {
	...
	resourceId = request.getParameter("resourceViewId");
        ...
        String[] specificIds = (String[]) rvs.get("specificIds");
        ...
        for(int i = 0; i < specificIds.length; i++) {
            SpecificIndustry si = ClientService.getInstance().getSingleSpecificIndustry(Integer.valueOf(specificIds[i]));
            ResourceService.getInstance().linkResourceSpecificIndustry(r, si);      
            
            //Insert general Industries as well for search purposes
            Industry ind = ClientService.getInstance().getSingleIndustry(si.getIndustry().getIndustryId());
            ResourceService.getInstance().linkResourceIndustry(r, ind);
