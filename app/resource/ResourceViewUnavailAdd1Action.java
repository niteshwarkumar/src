//ResourceViewUnavailAdd1Action.java adds the 
//new unavailability
...
public final class ResourceViewUnavailAdd1Action extends Action {
	...
	resourceId = request.getParameter("resourceViewId");
        ...
        //values for adding a new unavail from form
        DynaValidatorForm rvt = (DynaValidatorForm) form;
        Unavail unavailNew = (Unavail) rvt.get("unavailNew");
        
        String unavailStart = request.getParameter("unavailStartNew");
        if(unavailStart.length() > 0) {
            unavailNew.setStartDate(DateService.getInstance().convertDate(unavailStart).getTime());
        }
        
        String unavailEnd = request.getParameter("unavailEndNew");
        if(unavailEnd.length() > 0) {
            unavailNew.setEndDate(DateService.getInstance().convertDate(unavailEnd).getTime());
        }  
        
        //add to db
        ResourceService.getInstance().addUnavail(unavailNew, r);
