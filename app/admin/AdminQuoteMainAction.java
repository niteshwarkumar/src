//AdminQuoteMainAction.java gets quote-module related options
//ready
...
public final class AdminQuoteMainAction extends Action {
	...
	ActionMessages errors = new ActionMessages();
        ...
        //get quote related admin options
        List quoteOptions = AdminService.getInstance().getQuoteOptions();
                
        //place quote admin options into form
        DynaValidatorForm ad = (DynaValidatorForm) form;
        for(ListIterator iter = quoteOptions.listIterator(); iter.hasNext();) {
        ...
        for(int i = 0; i < usersArray.length; i++) {
            if(StandardCode.getInstance().checkPrivSet(usersArray[i].getPrivileges(), "quoteViewAccountTab")) {
                request.setAttribute("quoteViewAccountTab" + String.valueOf(i), "checked");
            }
            else {
                request.setAttribute("quoteViewAccountTab" + String.valueOf(i), "");
            }
            
            if(StandardCode.getInstance().checkPrivSet(usersArray[i].getPrivileges(), "quoteUpdateAccountTab")) {
                request.setAttribute("quoteUpdateAccountTab" + String.valueOf(i), "checked");
            }
            else {
                request.setAttribute("quoteUpdateAccountTab" + String.valueOf(i), "");
            }
        }
