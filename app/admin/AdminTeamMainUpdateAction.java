//AdminTeamMainUpdateAction.java gets team-module related options
...
public final class AdminTeamMainUpdateAction extends Action {
	...
	ActionMessages errors = new ActionMessages();
        ...
        for(int i = 0; i < usersArray.length; i++) {
            ...
            String teamViewAccountTab = request.getParameter("teamViewAccountTab" + String.valueOf(i));
            String teamUpdateAccountTab = request.getParameter("teamUpdateAccountTab" + String.valueOf(i));
            
            //update each privilege per user
            if(teamViewAccountTab != null) {
                if(!StandardCode.getInstance().checkPrivSet(usersArray[i].getPrivileges(), "teamViewAccountTab")) {
                    Privilege p = new Privilege();
                    p.setPrivilege("teamViewAccountTab");
            ...
            if(teamUpdateAccountTab != null) {
                ...
                for(Iterator iter = usersArray[i].getPrivileges().iterator(); iter.hasNext();) {
                    Privilege p = (Privilege) iter.next();
                    if(p.getPrivilege().equals("teamUpdateAccountTab")) {
                        UserService.getInstance().deletePrivilege(p);
                    }
                }
            }
