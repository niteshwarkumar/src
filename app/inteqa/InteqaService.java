...
public class InteqaService {
    ...
    if(inRep.isVerified()==false){return false;}
    return true;
    } 
    ...
    if(indel.isVerified()==false){return false;}
    if(indel.isEngverified()==false){return false;}
    if(indel.isDtpverified()==false){return false;}
    return true;
    } 
    ...
    List inDelReqList = getInDeliveryReqGrid(indel.getId());
    ...
    public List getInDeliveryReqGrid(int inDeliveryId) {
        /*
         * Use the ConnectionFactory to retrieve an open
         * Hibernate Session.
         *
        ...
        try {
          ...
          //  query = session.createQuery("select quote from app.quote.Quote1 quote where quote.status = 'pending' and DATEDIFF(" + date+",quote.quoteDate) > 120 order by quote.number ");
            ...
            query = session.createQuery("select inDeliveryReq from app.inteqa.INDeliveryReq inDeliveryReq where inDeliveryReq.inDeliveryId="+inDeliveryId);
            return query.list();
        } catch (HibernateException e) {
            System.err.println("Hibernate Exception" + e.getMessage());
            throw new RuntimeException(e);
