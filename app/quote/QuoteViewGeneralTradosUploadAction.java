...
public final class QuoteViewGeneralTradosUploadAction extends Action {
	...
	quoteId = request.getParameter("quoteViewId");
      ...
      String[] lines = entireRead.split("\n"); //lines within the file
       if(myFile.getFileName().toLowerCase().endsWith(".log")){
      ...
      fileData = myFile.getFileData(); //byte array of entire file
      in.read(fileData); //read data into fileData
      ...
      //look for files and add them to the db
      for(int i = 0; i < lines.length; i++) {
          line = lines[i];
          parts = line.split("\\\\");
          if(parts != null && parts.length > 1 && parts[parts.length-1].indexOf(".tmw") == -1) {
              boolean alreadyIn = false; //check if already in list
              for(Iterator iter = q.getFiles().iterator(); iter.hasNext();) {
                  File f = (File) iter.next();
                  if(f.getFileName().equals(parts[parts.length-1].trim())) {
                      alreadyIn = true;
                  }
              }
              
              if(alreadyIn == false) {
                   //add this file to the quote's file list in db
                   File f = new File();
                   f.setFileName(parts[parts.length-1].trim());
                   f.setBeforeAnalysis("false");
                   QuoteService.getInstance().addFileWithQuote(q, f);
              }
          }
      }
