...
public final class QuoteViewGeneralApprovedAction extends Action {
  ...
  public ActionForward execute(ActionMapping mapping,
    ...
    ProjectService.getInstance().unlinkAllProjectTechnical(p.getProjectId());
    //List ptechList = QuoteService.getInstance().getTechnicalList(0);
    ...
    for (int i = 0; i < clientQuoteList.size(); i++) {
      ...
      List ptechList = QuoteService.getInstance().getTechnicalList(clientQuote.getId());
      for(int t = 0; t<ptechList.size(); t++){
          Technical tech = (Technical) ptechList.get(t);
          Project_Technical pt = new Project_Technical();
          pt.setProjectid(p.getProjectId());
          pt.setSourceapp(tech.getSourceApplication());
          pt.setSourceos(tech.getSourceOs());
          pt.setSourcever(tech.getSourceVersion());
          pt.setTargetapp(tech.getTargetApplication());
          pt.setTargetos(tech.getTargetOs());
          pt.setTargetver(tech.getTargetVersion());
          pt.setUnitCount(tech.getUnitCount());
    ...
    q.setApprovedTS(new java.sql.Timestamp(System.currentTimeMillis()));
...
//          subQ.setProject(null);
          subQ.setApprovalDate(new Date());
//          subQ.setSubQuoteId(id);
          QuoteService.getInstance().updateQuote(subQ,(String)request.getSession(false).getAttribute("username"));
...
//            }
            ...
            if (linTasks != null) {
              for (Iterator linIter = linTasks.iterator(); linIter.hasNext();) {
                ...
                if (p.getCompany().isScaleDefault(p.getProjectId())) {
                  ...
                  try {
                    scalePerfect = new Double(q.getProject().getCompany().getScalePerfect(p.getProjectId(),p.getCompany().getClientId())).doubleValue();
                    scaleContext = new Double(q.getProject().getCompany().getScaleContext(p.getProjectId(),p.getCompany().getClientId())).doubleValue();
                  } catch (Exception e) {
                  }
    ...
    try {
      ...
      CommService.getInstance().addProjectRequirementOnQuoteApprove(p.getProjectId(),client.getClientId());
    request.setAttribute("ae", ae.getWorkEmail1());
    ...
    String emailSubjectTxt = "Quote Approval: " + q.getNumber() + " (project " + p.getNumber() + p.getCompany().getCompany_code() + ") - Quote Approval - "+p.getCompany().getCompany_name();
    ...
    if (u.getuserType() != null && u.getuserType().equalsIgnoreCase("client")) {
      ...
      if (null != p.getCompany().getAuto_alert()) {
        ccAddress += "," + p.getCompany().getAuto_alert();
      }
      ...
      emailMsgTxt = "<div style='font-family: Verdana; font-size:10pt'><div>Hello,</div><div>&nbsp;</div><div>This message is automatically generated by ExcelNet."
              ...
              + "<div><table style='font-family: Verdana; font-size:10pt'><tr><td style='padding-right: 15px;'>AE: </td><td>" + aeName + "</td></tr>"
              ...
              + "<tr><td>&nbsp;</td></tr>"
              + "<tr><td>Source: </td><td> " + sourceLang + "</td></tr>"
              ...
              + "<div>&nbsp;</div><div>ExcelNet Administrator</div><div><img src='https://excelnet.xltrans.com/logo/images/excel-logo-blue.jpg'></div></div>";
    } else {
      emailMsgTxt = "<div style='font-family: Verdana; font-size:10pt'><div>Hello,</div><div>&nbsp;</div><div>This message is automatically generated by ExcelNet.</div><div>&nbsp;</div>"
              + "<div><strong>" + q.getNumber() + " </strong>has been approved: (project " + p.getNumber() + p.getCompany().getCompany_code() + ")"
              + "</div><div>&nbsp;</div><div>=============================</div><div>&nbsp;</div>"
              + "<div><table style='font-family: Verdana; font-size:10pt'><tr><td style='padding-right: 15px;'>AE: </td><td>" + aeName + "</td></tr>"
              + "<tr><td>PM:</td><td> " + pmName + "</td></tr>"
    ...
    for (int i = 0; i < tickerList.size(); i++) {
      ...
      if (!temp.contains(ticker.getUserEmail())) {
        if (ccAddress.equalsIgnoreCase("")) {
          try {
            ccAddress = ticker.getUserEmail();
          } catch (Exception e) {
          }
        } else {
          try {
...
//        toAddress +="";
    ...
    String[] ccEmailList = ccAddress.split(",");
    temp = new HashSet<String>(Arrays.asList(ccEmailList));
    temp.remove("");
    ccEmailList = temp.toArray(new String[temp.size()]);

    SendEmail smtpMailSender = new SendEmail();
      //System.out.println(emailList);

        smtpMailSender.postMail(emailList,ccEmailList, emailSubjectTxt, emailMsgTxt,
