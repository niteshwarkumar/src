//ReportsList2Action.java gets the display
...
public final class ReportsList2Action extends Action {
	...
	ActionMessages errors = new ActionMessages();
        ...
        //run report search
        List results = ReportsService.getInstance().runReport2(dateA, dateB);
        ...
        if(results != null) {
            for(ListIterator iter = results.listIterator(); iter.hasNext();) {
                ...
                if(p.getProjectAmount() != null) {
                    ...
                    if(p.getStatus().equalsIgnoreCase("active")||p.getStatus().equalsIgnoreCase("complete")){
                    totalActive += p.getProjectAmount().doubleValue();
                    }
                }
            }
        }
        request.setAttribute("total", StandardCode.getInstance().formatDouble(new Double(total)));
        request.setAttribute("totalActive", StandardCode.getInstance().formatDouble(new Double(totalActive)));
        
        
        request.setAttribute("title", "All Projects from All Clients from " + DateFormat.getDateInstance(DateFormat.SHORT).format(dateA) + " to " + DateFormat.getDateInstance(DateFormat.SHORT).format(dateB) + resultsSize);
