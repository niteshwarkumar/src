//ReportsList1Action.java gets the display
...
public final class ReportsList1Action extends Action {
	...
	ActionMessages errors = new ActionMessages();
        ...
        Client c;
        if(clientId != null) {
            c = ClientService.getInstance().getSingleClient(Integer.valueOf(clientId));
        }
        else { //default to first in db
            List results = ClientService.getInstance().getClientList();
            c = (Client) results.get(0);
            clientId = String.valueOf(c.getClientId());
        }
        ...
        //run report search
        List results = ReportsService.getInstance().runReport1(c, dateB, dateC);
        ...
        request.setAttribute("title", "All Projects from " + c.getCompany_name() + " from " + DateFormat.getDateInstance(DateFormat.SHORT).format(dateB) + " to " + DateFormat.getDateInstance(DateFormat.SHORT).format(dateC) + resultsSize);
